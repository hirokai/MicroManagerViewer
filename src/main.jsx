...
var LeftPane = React.createClass({
    ...
    render() {
        const self = this;
        const datasets = this.state.filter ? _.pick(this.props.datasets, (d, k) => d.name.indexOf(self.state.filter) != -1) :
            this.props.datasets;
        console.log(this.state.filter, datasets);
        return <div className="col-md-3">
            <p>Choose dataset</p>
            <input onChange={this.onChangeDatasetFilter}/>
            <ul id='data-menu' className="nav nav-pills nav-stacked">
                    {_.map(datasets, function (d, k) {
                        return <DataSetEntry key={k} data={d} uuid={k}
                            active={d.uuid == (self.props.currentDataset ? self.props.currentDataset.uuid : null)}
    ...
    onClickDataSet(uuid) {
        this.props.onChangeDataset(uuid);
    },
    onChangeDatasetFilter(ev) {
...
var App = React.createClass({
    getInitialState() {
        return {datasets: {}, allDataSets: {}, currentDataset: null, dims: {}};
    },
    render() {
        return <div>
            <LeftPane datasets={this.state.allDataSets} onChangeDataset={this.onChangeDataset}
    ...
    onChangeDataset(uuid) {
        this.setState({currentDataset: this.state.allDataSets[uuid]});
//        console.log("onChangeDataset", this.state.allDataSets[uuid]);
    },
    componentDidMount: function () {
        ...
        d3.csv('datasets.csv', function (d) {
            ...
            console.log(ds);
            self.setState({allDataSets: ds, datasets: ds, currentDataset: ds[0]});
        });
    }
...
var RightPane = React.createClass({
    ...
    coordChanged(v) {
        ...
        this.setState({
            coord: newCoord,
            selectedImages: this.selectImages(this.state.images, this.state.filterDims, newCoord),
            remainingDim: this.calcRemainingDim()
        });
    },
    calcRemainingDim() {
    ...
    filterDimChanged(filterDims) {
        ...
        this.setState({
            ...
            remainingDim: this.calcRemainingDim(),
            coord: newCoord,
            selectedImages: this.selectImages(this.state.images, filterDims, newCoord)
        });
    },
...
//        console.log(Object.values(images).length,f,coord);
    ...
    datasetChanged() {
        ...
        function convertCoord(s, imgs) {
            ...
            var prv_c = 0;
            var max_i = 0;
            var imgs2 = _.sortBy(imgs, img => {
                return img['meta_' + s.metasetdim];
            });
...
//            console.log("datasetChanged(): metadata loaded.", error, imgs);
            ...
            var dims = {};
            dims.pos = s.metaset ? (_.max(_.map(imgs, im => {
                return im.pos;
            })) + 1) : s.positions;
            dims.frame = s.metaset ? (_.max(_.map(imgs, im => {
                return im.frame;
            })) + 1) : s.frames;
            dims.ch = s.metaset ? (_.max(_.map(imgs, im => {
                return im.ch;
            })) + 1) : s.channels;
            dims.slice = s.metaset ? (_.max(_.map(imgs, im => {
                return im.slice;
            })) + 1) : s.slices;

            s.dims = dims;

            console.log(dims, _.sortBy(Object.keys(dims), function (k) {
                return -dims[k]
            })[0]);
            var biggestDim = _.sortBy(Object.keys(dims), function (k) {
                return -dims[k]
            })[0];
            var coord = {};
            coord[biggestDim] = 0;
...
var Slider = React.createClass({
    ...
    componentDidMount: function () {
        this.slider = $('#' + this.props.dim + '-slider-input').slider({
            min: 0,
            max: this.props.max,
...
var MapTools = React.createClass({
    ...
    render: function () {
        return <div id='map-tools'>
            <span style={{'margin': '0px 10px 10px 20px'}}>Mapping</span>
    ...
    xyTool: function () {
        ...
        return <div id="xy-tool">
            ...
            <button id="switch-xy" className='btn btn-xs btn-default' style={{
                cursor: 'pointer',
                margin: '0px 7px'
            }} onClick={this.onClickSwitch}>&nbsp;&#8596;&nbsp;</button>
